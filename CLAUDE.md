...
- **Save**: JSON serialized via the active `SaveStorage` adapter under key `gliese.save.v5` (`SAVE_STORAGE_KEY` in `src/lib/game/save/storage.ts`). In Tauri the adapter persists to `gliese-save.json` in the app-data directory; in a plain browser it falls back to `localStorage`. The payload's `version` field tracks the schema and is currently `5`; bump both `SAVE_STORAGE_KEY` and `version` and update `isSaveState` whenever `SaveState` changes shape.
