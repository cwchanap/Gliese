...
function validProvenance(): Record<string, unknown> {
	return JSON.parse(publicationPackage('validate').provenanceJson.toString('utf8'));
}

function validManifest(): Record<string, unknown> {
	return JSON.parse(publicationPackage('validate').cropManifestJson.toString('utf8'));
}
...
): MeadowEntryExportPackageBytes {
	const base = publicationPackage('validate');
	return {
		...
		cropManifestJson:
			typeof manifest === 'string' ? Buffer.from(manifest) : Buffer.from(JSON.stringify(manifest))
	};
}

async function writeRawSnapshot(packageBytes: MeadowEntryExportPackageBytes): Promise<string> {
	const root = await mkdtemp(join(tmpdir(), 'gliese-export-validate-'));
	await mkdir(join(root, 'exports'), { recursive: true });
	await mkdir(join(root, 'provenance'), { recursive: true });
	for (const [filename, bytes] of Object.entries(packageBytes.files)) {
		await writeFile(join(root, 'exports', filename), bytes);
	}
	await writeFile(
		join(root, 'provenance/meadow-entry-export-provenance.json'),
		packageBytes.provenanceJson
	);
	await writeFile(
		join(root, 'provenance/meadow-entry-crop-manifest.json'),
...
describe('meadow-entry export publication', () => {
	...
	it('rejects export filenames that are not basenames', async () => {
		const root = await mkdtemp(join(tmpdir(), 'gliese-export-basename-test-'));
		const base = publicationPackage('basename');
		const pkg: MeadowEntryExportPackageBytes = {
			files: { 'subdir/file.png': Buffer.from('test') },
			provenanceJson: base.provenanceJson,
			cropManifestJson: base.cropManifestJson
		};
		await expect(publishMeadowEntryExportPackage(root, pkg)).rejects.toThrow(
			/Refusing unexpected Meadow Entry export filename/i
		);
	});

	it('rejects export filenames that do not end in .png', async () => {
		const root = await mkdtemp(join(tmpdir(), 'gliese-export-png-test-'));
		const base = publicationPackage('png');
		const pkg: MeadowEntryExportPackageBytes = {
			files: { 'file.txt': Buffer.from('test') },
			provenanceJson: base.provenanceJson,
			cropManifestJson: base.cropManifestJson
		};
		await expect(publishMeadowEntryExportPackage(root, pkg)).rejects.toThrow(
	...
	it('exhausts all attempts and fails closed when the sentinel persists', async () => {
		const root = await mkdtemp(join(tmpdir(), 'gliese-export-sentinel-test-'));
		await publishMeadowEntryExportPackage(root, publicationPackage('sentinel'));
		const fileSystem: MeadowEntryExportSnapshotFileSystem = {
			lstat: async (path) => {
				if (String(path).endsWith('.meadow-entry-export-publication.lock')) {
					return {} as ReturnType<typeof lstat>;
				}
				return lstat(path);
			},
			readFile,
		...
		await expect(
			readPublishedMeadowEntryExportSnapshot(root, { attempts: 2, fileSystem })
		).rejects.toThrow(/publication is in progress/i);
	});

	it('succeeds on retry after a transient read failure', async () => {
		const root = await mkdtemp(join(tmpdir(), 'gliese-export-retry-test-'));
		const pkg = publicationPackage('retry');
		...
		const fileSystem: MeadowEntryExportSnapshotFileSystem = {
			...
			readFile: async (path) => {
				const pathString = String(path);
				if (pathString.endsWith('meadow-entry-export-provenance.json')) {
					provenanceReads += 1;
					if (provenanceReads === 1) throw new Error('transient read failure');
				}
				return readFile(path);
			}
		};
		const result = await readPublishedMeadowEntryExportSnapshot(root, { fileSystem });
		expect(result).toEqual(pkg);
	});
});

describe('meadow-entry export snapshot validation', () => {
	it('rejects provenance that is not valid JSON', async () => {
		const root = await writeRawSnapshot(packageFrom('{not valid json', validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/is not valid JSON/i
		);
	});

	it('rejects provenance that is a JSON array rather than an object', async () => {
		const root = await writeRawSnapshot(packageFrom('[]', validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/must be a JSON object/i
		);
	});

	it('rejects provenance whose version has drifted', async () => {
		const provenance = validProvenance();
		provenance.version = 2;
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/version has drifted/i
		);
	});

	it('rejects provenance missing the control fingerprint', async () => {
		const provenance = validProvenance();
		provenance.controls = {};
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/requires fingerprint/i
		);
	});

	it('rejects when the provenance and manifest control fingerprints differ', async () => {
		const provenance = validProvenance();
		provenance.controls.fingerprint = 'b'.repeat(64);
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/controls differ/i
		);
	...
	it('rejects a control fingerprint that is not a lowercase SHA-256 hash', async () => {
		const provenance = validProvenance();
		provenance.controls.fingerprint = 'not-a-valid-sha256';
		const manifest = validManifest();
		manifest.controlFingerprint = 'not-a-valid-sha256';
		const root = await writeRawSnapshot(packageFrom(provenance, manifest));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/must be a lowercase SHA-256 hash/i
		);
	});

	it('rejects when the provenance and manifest master identities differ', async () => {
		const manifest = validManifest();
		manifest.masters.baseSha256 = 'b'.repeat(64);
		const root = await writeRawSnapshot(packageFrom(validProvenance(), manifest));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/master identities differ/i
		);
	});

	it('rejects an invalid inventory entry that is not an object', async () => {
		const provenance = validProvenance();
		provenance.inventory = [1];
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
	...
	it('rejects an inventory entry with an invalid filename', async () => {
		const provenance = validProvenance();
		provenance.inventory = [{ filename: 'not-png.txt', bytes: 10, sha256: 'a'.repeat(64) }];
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/invalid filename/i
		);
	...
	it('rejects an inventory entry with invalid bytes', async () => {
		const provenance = validProvenance();
		provenance.inventory = [{ filename: 'fixture-base.png', bytes: 0, sha256: 'a'.repeat(64) }];
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/invalid bytes/i
		);
	...
	it('rejects an inventory entry with an invalid hash', async () => {
		const provenance = validProvenance();
		provenance.inventory = [{ filename: 'fixture-base.png', bytes: 10, sha256: 'not-a-hash' }];
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/invalid hash/i
		);
	...
	it('rejects duplicate filenames in the inventory', async () => {
		const provenance = validProvenance();
		provenance.inventory = [
			{ filename: 'fixture-base.png', bytes: 10, sha256: 'a'.repeat(64) },
			{ filename: 'fixture-base.png', bytes: 10, sha256: 'a'.repeat(64) }
		];
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
	...
	it('rejects when the inventory does not match the crop manifest filenames', async () => {
		const provenance = validProvenance();
		provenance.inventory = [{ filename: 'other.png', bytes: 10, sha256: 'a'.repeat(64) }];
		const root = await writeRawSnapshot(packageFrom(provenance, validManifest()));
		await expect(readPublishedMeadowEntryExportSnapshot(root, { attempts: 1 })).rejects.toThrow(
			/inventory differs/i
		);
...
describe('meadow-entry approved master snapshot guard', () => {
	...
	it('rejects provenance that is not valid JSON', () => {
		const fixture = approvedMasterFixture();
		const invalidJson = Buffer.from('{not valid json');
		expect(() =>
			assertApprovedMasterSnapshot(
				{ ...fixture.snapshot, provenanceJson: invalidJson },
				{ ...fixture.expected, provenanceSha256: sha256(invalidJson) }
			)
		).toThrow(/not valid JSON/i);
	});

	it('rejects provenance missing the base sha256', () => {
		const fixture = approvedMasterFixture();
		const provenance = {
		...
		expect(() =>
			assertApprovedMasterSnapshot(
				{ ...fixture.snapshot, provenanceJson },
				{ ...fixture.expected, provenanceSha256: sha256(provenanceJson) }
			)
		).toThrow(/plane hashes have drifted/i);
	});

	it('rejects when the provenance base plane hash has drifted', () => {
		const fixture = approvedMasterFixture();
		const provenance = {
			controls: { fingerprint: fixture.expected.controlFingerprint },
