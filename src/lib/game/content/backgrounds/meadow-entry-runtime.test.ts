...
describe('Meadow Entry PR-1 runtime projection', () => {
	it('activates exactly the approved Crossroads, connector, Wildwood, and east-boundary crops', () => {
		...
		expect(meadowEntryRuntimeBackgrounds).toHaveLength(16);
	...
	it('assigns fallback ownership only to rows with a complete active crop group', () => {
		...
		expect(activeMeadowEntryRuntimeVisualOwners).toHaveLength(66);
		expect(
			Object.fromEntries(
		...
		).toEqual({ blocker: 29, decor: 32, fence: 5 });
		...
		expect(
			MEADOW_ENTRY_RUNTIME_VISUAL_OWNERS.find(
				({ sourceId }) => sourceId === 'coast-approach-west-bank'
			)
		).toMatchObject({
			sourceType: 'blocker',
			ownerCrops: [{ cropId: 'tidewatch-coast' }]
		});
	});
