export const en = {
	ui: {
		menu: 'Menu',
		systemMenu: 'System Menu',
		command: 'Command',
		fieldStatus: 'Field status',
		settings: 'Settings',
		language: 'Language',
		system: 'System',
		systemTitle: 'Display & Text',
		systemLanguageHint: 'Interface and dialogue',
		railDisplay: 'Display',
		railAudio: 'Audio',
		railInput: 'Input',
		audioUnavailable: 'Audio settings are not available in this build.',
		textSpeed: 'Text speed',
		textSpeedHint: 'Dialogue reveal',
		speedSlow: 'Slow',
		speedNormal: 'Normal',
		speedInstant: 'Instant',
		hudDensity: 'HUD density',
		hudDensityHint: 'How much the field shows',
		densityQuiet: 'Quiet',
		densityFull: 'Full',
		densityFullUnavailable: 'Full density is not available yet.',
		motion: 'Motion',
		motionHint: 'Glints, sweeps, pulses',
		motionOn: 'On',
		motionReduced: 'Reduced',
		prompts: 'Prompts',
		promptsHint: 'Which button glyphs to show',
		promptAuto: 'Auto',
		promptPad: 'Pad',
		promptKeys: 'Keys',
		quests: 'Quests',
		map: 'Map',
		areaMap: 'Area Map',
		areaMapDialog: '{{areaName}} map',
		regionSubline: 'Verdant Region / Day 03',
		unexplored: 'Unexplored',
		currentPosition: 'Current position',
		areaMapSelectedMarker: 'Selected: {{name}}',
		heroName: 'Liam',
		goldSuffix: 'G',
		activeQuest: 'Active Quest',
		shop: 'Shop',
		inventory: 'Inventory',
		saveGame: 'Save Game',
		useHeal: 'Use Heal',
		fieldCmds: {
			bag: 'Bag',
			gear: 'Gear',
			quest: 'Quest',
			map: 'Map',
			skill: 'Skill',
			rest: 'Rest',
			save: 'Save',
			system: 'System'
		},
		titleChapterPill: 'Verdant Region · Chapter I',
		chapterLabel: 'Chapter I',
		titleNewRun: 'New Run',
		titleNewRunSub: 'Chapter I',
		titleSystemSub: 'Language · Audio',
		titleNoSaveData: 'No save data',
		titleHintSelect: 'Select',
		titleHintEnter: 'Enter',
		titleNewRunConfirm: 'Start a new run? The autosave will be overwritten.',
		saveScreen: 'Save',
		loadScreen: 'Load',
		waystone: 'Waystone',
		slotAutosave: 'Autosave',
		slotManual: 'Manual',
		slotEmpty: 'No record',
		saveHere: 'Save here',
		overwriteTitle: 'Overwrite this save?',
		confirmOverwrite: 'Overwrite',
		today: 'Today',
		yesterday: 'Yesterday',
		battleSummary: 'Battle Summary',
		battleVictory: 'Victory',
		battleDefeat: 'Defeat',
		battleHud: 'Battle',
		battleTurn: 'Turn',
		battleAuto: 'Auto',
		battleTargeted: 'Targeted',
		battleWaiting: 'Waiting',
		battleFeed: 'Battle feed',
		battleFeedHit: '−{{amount}} {{name}} hit',
		battleFeedHurt: '−{{amount}} {{name}} hurt',
		battleFeedHeal: '+{{amount}} {{name}}',
		battleFeedDefeat: '{{name}} defeated',
		battleHeal: 'Heal',
		battleItem: 'Item',
		battleFlee: 'Flee',
		battleOneCharge: '1 charge',
		battleCharges: '{{count}} charges',
		battleItemsInBag: '{{count}} in bag',
		battleStatXp: 'XP',
		battleStatCoins: 'Coins',
		battleStatDrop: 'Drop',
		battleStatFoes: 'Foes',
		enemiesDefeated: 'Enemies defeated: {{count}}',
		xpGained: 'XP gained: {{xp}}',
		coinsGained: 'Coins gained: {{coins}}',
		levelUp: 'Level up',
		noDrops: 'No item drops',
		continue: 'Continue',
		questProgressUpdate: '{{progressLabel}}: {{currentProgress}}/{{target}}',
		defeatReturnedToVillage: 'No rewards gained. Returned to the Shrine.',
		close: 'Close',
		next: 'Next',
		mainQuest: 'Main Quest',
		sideActive: '{{count}} side active',
		fieldPack: 'Field Pack',
		bagCategories: {
			potions: 'Potions',
			gear: 'Gear',
			key: 'Key',
			loot: 'Loot'
		},
		bagWorn: 'Worn',
		bagUseItem: 'Drink',
		bagEquipItem: 'Equip',
		bagSelectHint: 'Select an item to inspect.',
		bagPaperDollAlt: 'Liam full-body paper doll',
		equipped: 'Equipped',
		empty: 'Empty',
		key: 'Key',
		remove: 'Remove',
		fieldJournal: 'Field Journal',
		questLog: 'Quest Log',
		main: 'Main',
		side: 'Side',
		questOffered: 'Offered',
		questRewards: 'Reward',
		questTurnedInBy: 'Turned in by',
		questChapterProgress: 'Chapter Progress',
		questMapContext: 'Map Context',
		questJournalEmpty: 'No quests in the journal yet.',
		rewardCoins: 'Coins',
		rewardItem: 'Item',
		noSideQuestsActive: 'No side quests active.',
		coins: 'Coins: {{coins}}',
		buy: 'Buy',
		sell: 'Sell',
		noStockAvailable: 'No stock available.',
		noSellableItems: 'No sellable items.',
		buyFor: 'Buy for {{meta}}',
		sellFor: 'Sell for {{meta}}',
		unlimited: 'Unlimited',
		stockLeft: '{{count}} left',
		emptyInventorySlot: 'Empty inventory slot {{index}}',
		shopSections: 'Shop sections',
		questTracker: 'Quest tracker',
		playerStatus: 'Player status',
		merchant: 'Merchant',
		loadGameShellError: 'Unable to start the game shell.',
		levelAbbrev: 'LV',
		level: 'Level {{level}}',
		hp: 'HP',
		maxHp: 'MAX HP',
		xp: 'XP',
		attack: 'ATK',
		defense: 'DEF',
		heals: 'Heals',
		accept: 'Accept',
		back: 'Back',
		noSkillsYet: 'No skills learned yet',
		quantity: 'x{{quantity}}',
		statModifier: '{{stat}} +{{value}}',
		shopBuyMeta: '{{price}} coins / {{stock}}',
		shopDimmedHint: "Dimmed = can't afford",
		shopPurse: 'Purse',
		shopPurseAfter: 'Purse after',
		shopNotEnough: 'Not enough',
		soldOut: 'Sold out',
		shopReplaces: 'Replaces {{slot}} · {{current}}',
		shopEmptySlot: 'empty',
		shopOwned: 'Owned x{{quantity}}',
		shopSelectHint: 'Select an item to inspect.',
		shopBustAlt: '{{merchant}}, merchant portrait',
		dialogueBustAlt: '{{npc}}, dialogue portrait',
		shopSellMeta: '{{price}} coins',
		shopSellMetaWithQuantity: '{{price}} coins / x{{quantity}}',
		priceBadge: '{{price}}c',
		inventorySlotsLabel: '{{section}} inventory slots',
		equipmentSlots: {
			weapon: 'Weapon',
			head: 'Head',
			body: 'Body',
			hands: 'Hands',
			accessory: 'Accessory'
		},
		itemKinds: {
			consumable: 'consumable',
			equipment: 'equipment'
		}
	},
	status: {
		loadingGame: 'Loading game',
		battleStarted: 'Battle started',
		battleActive: 'Battle in progress',
		battleVictory: 'Battle victory',
		battleDefeat: 'Battle defeat',
		battleReturned: 'Returned from battle',
		battleLocked: 'Cannot do that during battle',
		noHealCharges: 'No heal charges left',
		itemCannotBeUsed: 'Item cannot be used',
		hpAlreadyFull: 'HP already full',
		recoveredHp: 'Recovered HP',
		strikeLanded: 'Strike landed',
		enemyDefeated: 'Enemy defeated',
		victoryRuinsCleared: 'Victory: ruins cleared',
		itemCannotBeEquipped: 'Item cannot be equipped',
		equippedItem: 'Equipped item',
		unequippedItem: 'Unequipped item',
		noShopNearby: 'No shop nearby',
		shopOpened: 'Shop opened',
		shopClosed: 'Shop closed',
		boughtItem: 'Bought {{itemName}}',
		soldItem: 'Sold {{itemName}}',
		noGuildQuestAvailable: 'No Guild quest available',
		questAccepted: 'Quest accepted',
		questComplete: 'Quest complete: {{questTitle}}',
		questAlreadyActive: 'Quest already active',
		questAlreadyComplete: 'Quest already complete',
		questNotAvailable: 'Quest not available',
		questCannotBeAccepted: 'Quest cannot be accepted',
		notEnoughCoins: 'Not enough coins',
		itemOutOfStock: 'Item out of stock',
		itemCannotBeBought: 'Item cannot be bought',
		equippedItemCannotBeSold: 'Equipped item cannot be sold',
		itemNotOwned: 'Item not owned',
		itemCannotBeSold: 'Item cannot be sold',
		noDialogueOpen: 'No dialogue open',
		noOneNearby: 'No one nearby',
		ruinsRouteUnlocked: 'Ruins route unlocked',
		dialogueClosed: 'Dialogue closed',
		dialogueUpdated: 'Dialogue updated',
		saved: 'Saved',
		savedWithoutThumbnail: 'Saved without preview image',
		saveFailed: 'Could not save',
		saveResumed: 'Save resumed',
		enteredArea: 'Entered area',
		newRun: 'New run',
		reportToGuildMasterFirst: 'Report to the Guild Master first',
		shopOutOfReach: 'Shop out of reach',
		bossEnraged: 'Boss enraged',
		heroDown: 'Hero down',
		enemyStruckFirst: 'Enemy struck first',
		foundItem: 'Found {{itemName}}',
		npcNearby: '{{npcName}} nearby'
	},
	content: {
		enemies: {
			'slime-scout': {
				name: 'Slime Scout'
			},
			'ruins-warden': {
				name: 'Ruins Warden'
			}
		},
		items: {
			'field-potion': {
				name: 'Field Potion',
				description: 'Restores 8 HP.'
			},
			'greater-field-potion': {
				name: 'Greater Field Potion',
				description: 'Restores 14 HP.'
			},
			'ember-tonic': {
				name: 'Ember Tonic',
				description: 'Restores 5 HP.'
			},
			'ruin-draught': {
				name: 'Ruin Draught',
				description: 'Restores 10 HP.'
			},
			'sunleaf-salve': {
				name: 'Sunleaf Salve',
				description: 'Restores 6 HP.'
			},
			'training-sword': {
				name: 'Training Sword',
				description: 'A reliable starter blade.'
			},
			'ruin-blade': {
				name: 'Ruin Blade',
				description: 'A chipped sword humming with old heat.'
			},
			'iron-cap': {
				name: 'Iron Cap',
				description: 'Simple protection for dangerous ruins.'
			},
			'warden-crown': {
				name: 'Warden Crown',
				description: 'A cracked helm from the ruins core.'
			},
			'traveler-vest': {
				name: 'Traveler Vest',
				description: 'Light armor for long walks.'
			},
			'stone-mail': {
				name: 'Stone Mail',
				description: 'Heavy plates carved from ruin stone.'
			},
			'grip-wraps': {
				name: 'Grip Wraps',
				description: 'Cloth wraps that steady each strike.'
			},
			'meadow-charm': {
				name: 'Meadow Charm',
				description: 'A small charm from the meadow path.'
			},
			'meadow-token': {
				name: 'Meadow Token',
				description: 'A keepsake from the entry meadow.'
			},
			'threshold-rune': {
				name: 'Threshold Rune',
				description: 'A carved marker from the ruin threshold.'
			},
			'warden-sigil': {
				name: 'Warden Sigil',
				description: 'Proof that the ruins warden fell.'
			}
		},
		shops: {
			'miras-item-shop': {
				name: "Mira's Item Shop",
				merchantName: 'Mira',
				description: 'Reliable field supplies for the road east.'
			},
			'guild-quartermaster': {
				name: 'Guild Quartermaster',
				merchantName: 'Quartermaster Vale',
				description: 'Guild-approved gear for new ruins assignments.'
			},
			'sundrop-forge': {
				name: 'Sundrop Forge',
				merchantName: 'Blacksmith Oren',
				description: 'Village-forged equipment for the road beyond Sundrop.'
			}
		},
		quests: {
			'investigate-the-ruins': {
				title: 'Investigate the Ruins',
				description: 'Report to the Guild Master, then defeat the ruins warden.',
				objectives: {
					'talk-to-guild-master': {
						description: 'Talk to the Guild Master in the Guild Hall.',
						progressLabel: 'Guild Master spoken to'
					},
					'defeat-ruins-warden': {
						description: 'Defeat the ruins warden in the ruins core.',
						progressLabel: 'Ruins warden defeated'
					}
				}
			},
			'thin-village-slimes': {
				title: 'Thin Village Slimes',
				description: 'Clear the slimes gathering on the village road.',
				objectives: {
					'defeat-village-slimes': {
						description: 'Defeat slimes near the village.',
						progressLabel: 'Village slimes defeated'
					}
				}
			},
			'thin-ruins-slimes': {
				title: 'Thin Ruins Slimes',
				description: 'Reduce the slime presence inside the ruin threshold.',
				objectives: {
					'defeat-ruins-slimes': {
						description: 'Defeat slimes in the ruins.',
						progressLabel: 'Ruins slimes defeated'
					}
				}
			},
			'recover-ruins-relics': {
				title: 'Recover Ruins Relics',
				description: 'Bring back useful items from the old ruins.',
				objectives: {
					'collect-ruins-items': {
						description: 'Collect ruins items from the threshold and core.',
						progressLabel: 'Ruins items recovered'
					}
				}
			}
		},
		dialogue: {
			actions: {
				quest: 'Quest',
				shop: 'Shop',
				accept: 'Accept',
				back: 'Back',
				close: 'Close'
			},
			speakers: {
				traveler: 'Traveler',
				guildNotice: 'Guild Notice'
			},
			system: {
				noDialogueAvailable: 'No dialogue is available.',
				optionNotAvailable: 'That option is not available.',
				chooseGuildWork: 'Choose the Guild work you want to review.',
				noNewGuildWork: 'No new Guild work is available right now.',
				guildWorkUnavailable: 'That Guild work is no longer available.',
				noDialogueOpen: 'No dialogue is open.',
				noOneNearby: 'No one is nearby.',
				noGuildQuestAvailableHere: 'No Guild quest is available here.',
				shopOutOfReach: 'Shop out of reach.',
				questDetailNotice: '{{questTitle}}: {{objectiveDescription}} Reward: {{rewardSummary}}.',
				questCompleteNotice: 'Quest complete: {{questTitle}}. Reward: {{rewardSummary}}.'
			}
		},
		maps: {
			landmarks: {
				'hero-house-exterior': { label: "Hero's House" },
				'guild-hall-exterior': { label: 'Guild' },
				'item-shop-exterior': { label: 'Item Shop' },
				'villager-house-1-exterior': { label: 'Villager Home' },
				'villager-house-2-exterior': { label: 'Villager Home' },
				'villager-house-3-exterior': { label: 'Villager Home' },
				'sundrop-well': { label: 'Sundrop Well' },
				'whispering-cave': { label: 'Whispering Cave' },
				blacksmith: { label: 'Blacksmith' },
				'shrine-of-aurora': { label: 'Shrine of Aurora' },
				'witchwood-gate': { label: 'Witchwood Gate' },
				'silver-shrine-gate': { label: 'Silver Shrine Gate' },
				'ferry-crossing': { label: 'Ferry Crossing' },
				'castle-gate': { label: 'Sealed Castle Gate' },
				'crossroads-waystone': { label: 'Crossroads Waystone' }
			},
			discoveries: {
				'crossroads-waystone-sign': {
					label: 'Worn Waystone',
					description: 'Carved arrows fan out: shrine north, coast south, marsh west, cave east.'
				},
				'castle-gate-warning': {
					label: 'Castle Gate Notice',
					description:
						'The royal road is sealed by decree. None pass the white line until the festival ends.'
				},
				'ferry-shrine-lore': {
					label: 'Ferry Shrine',
					description:
						'Ring the bell at dusk and the ferryman answers — when the tide and the war allow.'
				},
				'coast-jetty-foreshadow': {
					label: "Jetty's End",
					description: 'The planks stop at open water. No boat waits here yet.'
				},
				'witchwood-poison-warning': {
					label: 'Witchwood Marker',
					description: 'Breathe shallow past the gate. The blooms here weep a poison mist.'
				},
				'silverpine-amulet-foreshadow': {
					label: 'Amulet Rack',
					description: 'Pilgrims hang charms to seal a pact older than the shrine.'
				},
				'wildwood-cave-danger': {
					label: 'Cave Warning',
					description: 'Claw-scarred bark. Something denned in the Whispering Cave does not sleep.'
				}
			},
			npcs: {
				'guild-master': { name: 'Guild Master Arlen' },
				'guild-quartermaster': { name: 'Quartermaster Vale' },
				'blacksmith-oren': { name: 'Blacksmith Oren' },
				'shopkeeper-mira': { name: 'Mira' },
				'villager-lynn': { name: 'Lynn' },
				'villager-toma': { name: 'Toma' },
				'villager-io': { name: 'Io' }
			},
			areas: {
				'meadow-entry': 'Sundrop Meadows',
				'hero-house': "Hero's House",
				'guild-hall': 'Guild Hall',
				'item-shop': 'Item Shop',
				'blacksmith-interior': 'Sundrop Forge',
				'villager-house-1': 'Villager House',
				'villager-house-2': 'Villager House',
				'villager-house-3': 'Villager House',
				'shrine-of-aurora-interior': 'Shrine of Aurora',
				'ruins-threshold': 'Ruins Threshold',
				'ruins-core': 'Ruins Core'
			}
		},
		rewards: {
			xp: '{{xp}} XP',
			coins: '{{coins}} coins',
			oneItem: '{{count}} item',
			itemCount: '{{count}} item',
			separator: ' / '
		}
	}
} as const;

export type EnglishMessages = typeof en;
