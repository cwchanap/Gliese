...
camera-envelope, and two-crop runtime contracts sealed. Add a candidate-only deterministic apron
compositor around the existing core, use it to review seven fresh donor images, and switch production
finalization only after the user approves the exact candidate master. Publication then extends the
existing bake-ownership generator to the ten sealed blocker IDs and regenerates the same two runtime
textures; no new plane, descriptor, collision, or renderer branch is introduced.
...
- Consumes: existing `MeadowEntryPaintedV2SceneryMaskSet`, decoded panels/inserts,
  `meadowEntrySceneryInsetDistances`, `meadowEntryDetailFeatherWeight`, and
...
export interface MeadowEntryPaintedV2OrganicSceneryBakeResult
...
- The existing `enrichMeadowEntryPaintedV2Sources` remains the production entry point in this task;
  the checked-in master and package therefore remain byte-stable until Task 3.
...
- core-only output bytes, row metrics, topology requests, and topology hash exactly equal the legacy
...
- reversed decoded insert input order produces identical panels and provenance hashes;
...
exclusion. Hash candidate, allowed, distance, and final sparse weight records with the existing
stable SHA helpers. Keep these arrays local and return only hashes/counts in provenance.
...
pixel, local means, owner pixel, and channel cap. Call it with cap `32/48` for core and `12` for
apron. Candidate apron contributions use each insert's existing owner priority and resolve collisions
by `(weight desc, ownerPriority desc, insertId asc)`.

Blend an apron channel using the existing half-up channel blender. Assert the resulting apron pixel
has per-channel delta `<=12` and luma delta `<=16` from the pre-scenery owner. Compose the shaped
core first, then apron, and return the ordinary panel list plus required apron provenance. Do not
pass apron contributions to `shapeMeadowEntryPaintedV2SceneryContributions`.

- [ ] **Step 5: Add candidate review mode without changing production finalization**

Update the review renderer to call
`enrichMeadowEntryPaintedV2SourcesWithOrganicApron` in candidate mode and emit:
...
rtk bun tools/render-meadow-entry-painted-v2-enrichment-review.ts \
  ...
  --contact-sheets \
  --source-review \
  --output-root docs/superpowers/reports/img/hpa-586-painted-v2-organic-scenery
```
...
Switch the finalizer's single scenery-enrichment call to
`enrichMeadowEntryPaintedV2SourcesWithOrganicApron`. Extend master/root provenance with the exact
