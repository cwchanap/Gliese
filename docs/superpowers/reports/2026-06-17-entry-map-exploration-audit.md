...
- `link-crossroads-silverpine` in `paths.ts` (`x: 3300, y: 2950, width: 500, height: 64`) spans x=3050 to x=3550 — its east end overlaps the festival road at x=3500 and its west end lands at x=3050, directly beneath `silverpine-stair-path` (`x: 3100, y: 1600, width: 70, height: 2400`). The stair-path's south end is at y=2800 (center y=1600 + half height=1200), while the connector sits at y=2950, leaving an unbridged vertical gap of approximately 150 px between y=2800 and y=2950 at x≈3050–3135. There is no named connector in `paths.ts` or `silverpine.ts` closing this gap; travel from the horizontal connector up to the stair-path crosses open terrain.
...
**Approach clues:**
- `link-crossroads-wildwood` in `paths.ts` (`x: 4000, y: 4300, width: 64, height: 1100`) is a vertical strip at x=4000, spanning y≈3750 to y≈4850. It does **not** connect directly to `sundrop-forest-road-north` (`x: 5600, y: 3200, width: 70, height: 4300`), which runs at x=5600. There is an unbridged horizontal gap of approximately 1600 px between x=4000 and x=5600; no named path connector in `paths.ts` or `wildwood.ts` crosses this gap. The route between the two road segments relies on open terrain rather than a defined path tile.
